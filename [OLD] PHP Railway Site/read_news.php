...
<div id="header">
          ...
          <div class="collapse navbar-collapse" id="navbar-collapse-2">
            ...
            <div class="collapse nav navbar-nav nav-collapse slide-down" id="nav-collapse3">
              <ul class="nav navbar-nav navbar-left">
                <?php if($u['email'] == "Administrator@koleje.pl"){ ?>
  ...
  <div class="container">          
      <div class="col-md-10 news-news pull-left">
    <?php 
    	$new=mysql_query("SELECT * FROM news WHERE Id='".$id."'");
    	$newnew=mysql_fetch_array($new); {
    ?>
    <h2><?php echo $newnew['title'] ?></h2>
    <h3><?php echo $newnew['Header'] ?></h3>
    <p><?php echo $newnew['Content'] ?></p>
    <h6>Dodane dnia <?php echo $newnew['Date'] ?></h6>
      ...
      <h2 class="page-header">Komentarze</h2>
          ...
          <article class="row">
			<form action="" method="post" autocomplete="off">
			  ...
			  <button type="submit" class="btn btn-default pull-right">Wyslij</button></br></br></br>
			</form>
		</article>
					<?php } ?>

    <?php
      $res=mysql_query("SELECT c.date, c.Content, u.UserName, u.Name FROM comments c, users u WHERE News_ID='".$id."' AND c.User_ID=u.Id");
    ...
    ?>
            ...
            <div class="col-md-2 col-sm-2 hidden-xs">
              <figure class="thumbnail">
                ...
                <figcaption class="text-center"><?php echo $row['UserName']?></figcaption>
              </figure>
            </div>
              ...
              <div class="panel panel-default arrow left">
                <div class="panel-body">
                  <header class="text-left">
                    <div class="comment-user"><i class="glyphicon glyphicon-user" aria-hidden="true"></i><?php echo $row['Name']?></div>
                    <time class="comment-date"><i class="glyphicon glyphicon-time" aria-hidden="true"></i><?php echo $row['date']?></time>
                  </header>
            	      <div class="comment-post">
                    <p>
                      </br><?php echo $row['Content']?></br>
                    </p>
                  </div>
                </div>
              </div>
...
<?php 
	...
	$userid = $u['Id'];
	$add_date = date('Y-n-j',mktime());
	
		if(isset($comment)){
			if($comment){
						mysql_query("INSERT INTO comments (`User_ID`, `News_ID`, `date`, `Content`) VALUES ('".$userid."','".$id."','".$add_date."','".$comment."')") or die(mysql_error());
					

						if(mysql_num_rows(mysql_query("SELECT * FROM comments WHERE User_ID='".$userid."' AND News_ID='".$id."' AND Content='".$comment."'")) != 0) {
						echo "Twoj komentarz zostal dodany, odswiez strone";
						} else  echo "Wystąpił błąd, spróbuj ponownie";
			} else echo "Napisz komentarz";
		} else echo "Napisz komentarz";
	}		
}?>
</div>
